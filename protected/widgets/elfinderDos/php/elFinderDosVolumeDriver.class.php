<?php
/**
 * Base class for elfinderDos volume.
 * Provide 2 layers:
 ...
 **/
abstract class elfinderDosVolumeDriver {
	...
	public function name() {
		return strtolower(substr(get_class($this), strlen('elfinderDosdriver')));
	}
	...
	public function mount(array $opts) {
		...
		$this->id = $this->driverId.(!empty($this->options['id']) ? $this->options['id'] : elfinderDos::$volumesCnt++).'_';
	...
	public function file($hash, $hidden=false) {
		...
		|| !($hidden || empty($file['hidden']))) {
			return $this->setError(elfinderDos::ERROR_FILE_NOT_FOUND);
		}
		return $file;
	}
	...
	public function dir($hash, $hidden=false, $resolveLink=false) {
		if (($dir = $this->file($hash, $hidden)) == false) {
			return $this->setError(elfinderDos::ERROR_DIR_NOT_FOUND);
		}
		
		if ($dir['mime'] != 'directory') {
			return $this->setError(elfinderDos::ERROR_NOT_DIR);
		}
		...
		return $dir['mime'] == 'directory' 
			? $dir 
			: $this->setError(elfinderDos::ERROR_NOT_DIR);
	}
	...
	public function scandir($hash, $mimes=array()) {
		...
		return $dir['read']
			...
			: $this->setError(elfinderDos::ERROR_PERM_DENIED);
	}
	...
	public function tree($hash='', $deep=0, $exclude='') {
		...
		if (!$dir['read']) {
			return $this->setError(elfinderDos::ERROR_PERM_DENIED);
		}
		$path = $this->decode($hash);
	...
	public function parents($hash) {
		...
		while ($path && $path != $this->root) {
			...
			if (!$this->attr($path, 'read')) {
				return $this->setError(elfinderDos::ERROR_OPEN, $this->_basename($path), '<br>', elfinderDos::ERROR_PERM_DENIED);
			}
	...
	public function mkdir($dst, $name, $copy=false) {
		...
		if (($dir = $this->dir($dst)) == false) {
			return $this->setError(elfinderDos::ERROR_TRGDIR_NOT_FOUND, '#'.$dst);
		}
		...
		if (!$this->nameAccepted($name)) {
			return $this->setError(elfinderDos::ERROR_INVALID_NAME);
		}
		...
		if ($this->_fileExists($dst)) {
			...
			return $this->setError(elfinderDos::ERROR_EXISTS, $name);
		}
	...
	public function mkfile($dst, $name) {
		...
		if (!$this->nameAccepted($name)) {
			return $this->setError(elfinderDos::ERROR_INVALID_NAME, $name);
		}
	...
	public function rename($hash, $name) {
		...
		if ($this->attr($path, 'locked')) {
			return $this->setError(elfinderDos::ERROR_LOCKED, $file['name']);
		}
		...
		if ($this->_fileExists($dst)) {
			return $this->setError(elfinderDos::ERROR_EXISTS, $file['name']);
		}
	...
	public function uploadAllow($tmpPath, $name) {
		...
		if (!($this->uploadOrder[0] == 'allow' ? $allow && !$deny : $allow || !$deny)) {
			return $this->setError(elfinderDos::ERROR_MIME);
		}
		
		if ($this->uploadMaxSize > 0 && filesize($tmpPath) > $this->uploadMaxSize) {
			return $this->setError(elfinderDos::ERROR_UPLOAD_SIZE);
		}
	...
	public function getContents($hash) {
		...
		if ($file['mime'] == 'directory') {
			return $this->setError(elfinderDos::ERROR_NOT_FILE);
		}
	...
	public function extract($hash) {
		...
		$archiver = isset($this->archivers['extract'][$file['mime']])
			...
			: false;
		if (!$archiver) {
			return $this->setError(elfinderDos::ERROR_NOT_ARCHIVE);
		}
	...
	public function archive($hashes, $mime) {
		...
		$archiver = isset($this->archivers['create'][$mime])
			...
			: false;
		if (!$archiver) {
			return $this->setError(elfinderDos::ERROR_ARCHIVE_TYPE);
		}
		...
		foreach ($hashes as $hash) {
			if (($file = $this->file($hash)) == false) {
				return $this->error(elfinderDos::ERROR_FILE_NOT_FOUND, '#'+$hash);
			}
			if (!$file['read']) {
				return $this->error(elfinderDos::ERROR_PERM_DENIED);
			}
			$path = $this->decode($hash);
			if (!isset($dir)) {
				$dir = $this->_dirname($path);
	...
	protected function doRm($path) {
		...
		if ($this->attr($path, 'locked')) {
			return $this->setError(elfinderDos::ERROR_LOCKED, $name);
		}
	...
	protected function doCopy($source, $dst, $name='') {
		...
		if (!$this->_isDir($dst)) {
			return $this->setError(elfinderDos::ERROR_TRGDIR_NOT_FOUND, $dst['name']);
		}
		...
		if ($this->_inpath($dst, $source)) {
			return $this->setError(elfinderDos::ERROR_COPY_INTO_ITSELF, $source['name']);
		}
