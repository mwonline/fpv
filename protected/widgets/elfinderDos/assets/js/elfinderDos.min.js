...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/elfinderDos.js
 */

/**
 * @class elfinderDos - file manager for web
 *
 * @author Dmitry (dio) Levashov
 **/
window.elfinderDos = function(nodeDos, opts) {
	this.time('load');
	
	var self = this,
		
		/**
		 * nodeDos on which elfinderDos creating
		 *
		 * @type jQuery
		 **/
		nodeDos = $(nodeDos),
		...
		/**
		 * Store nodeDos contents.
		 *
		 * @see this.destroy
		 * @type jQuery
		 **/
		prevContentDos = $('<div/>').append(nodeDos.contents()),
		
		/**
		 * Store nodeDos inline styles
		 *
		 * @see this.destroy
		 * @type String
		 **/
		prevStyleDos = nodeDos.attr('style'),
		 ...
		 * @type String
		 **/
		namespace = 'elfinderDos-'+(id || Math.random().toString().substr(2, 7)),
		...
		execShortcut = function(e) {
			...
			if (enabled) {
				...
				// prevent tab out of elfinderDos
	 ...
	 * @type String
	 **/
	this.cssClass = 'ui-helper-reset ui-helper-clearfix ui-widget ui-widget-content ui-corner-all elfinderDos elfinderDos-'+(this.direction == 'rtl' ? 'rtl' : 'ltr')+' '+this.options.cssClass;
	...
	this.draggable = {
		...
		drag       : function(e, ui) { ui.helper.toggleClass('elfinderDos-drag-helper-plus', e.shiftKey||e.ctrlKey||e.metaKey); },
		stop       : function() { self.trigger('focus').trigger('dragstop'); },
		helper     : function(e, ui) {
			var element = this.id ? $(this) : $(this).parents('[id]:first'),
				helper  = $('<div class="elfinderDos-drag-helper"><span class="elfinderDos-drag-helper-icon-plus"/></div>'),
				icon    = function(mime) { return '<div class="elfinderDos-cwd-icon '+self.mime2class(mime)+' ui-corner-all"/>'; },
				hashes, l;
			...
			if ((l = hashes.length) > 1) {
				helper.append(icon(files[hashes[l-1]].mime) + '<span class="elfinderDos-drag-num">'+l+'</span>');
			}
	...
	this.droppable = {
			tolerance  : 'pointer',
			accept     : '.elfinderDos-cwd-file-wrapper,.elfinderDos-navbar-dir,.elfinderDos-cwd-file',
	...
	this.enabled = function() {
		return nodeDos.is(':visible') && enabled;
	}
	...
	this.dialog = function(content, options) {
		return $('<div/>').append(content).appendTo(nodeDos).elfinderDosdialog(options);
	}
	
	/**
	 * Return UI widget or nodeDos
	 *
	 * @param  String  ui name
	 * @return jQuery
	 */
	this.getUI = function(ui) {
		return this.ui[ui] || nodeDos;
	}
	...
	this.resize = function(w, h) {
		nodeDos.css('width', w).height(h).trigger('resize');
		this.trigger('resize', {width : nodeDos.width(), height : nodeDos.height()});
	}
	
	/**
	 * Restore elfinderDos nodeDos size
	 * 
	 * @return elfinderDos
	 */
	...
	/**
	 * Destroy this elfinderDos instance
	 *
	 * @return void
	 **/
	this.destroy = function() {
		if (nodeDos && nodeDos[0].elfinderDos) {
			this.trigger('destroy').disable();
			listeners = {};
			shortcuts = {};
			$(document).add(nodeDos).unbind('.'+this.namespace);
			self.trigger = function() { }
			nodeDos.children().remove();
			nodeDos.append(prevContentDos.contents()).removeClass(this.cssClass).attr('style', prevStyleDos);
			nodeDos[0].elfinderDos = null;
		}
	}
	...
	// check nodeDos
	if (!nodeDos.length) {
		return alert(this.i18n('errnodeDos'));
	}
	// check connector url
	if (!this.options.url) {
	...
	this
		.enable(function() {
			if (!enabled && self.visible() && self.ui.overlay.is(':hidden')) {
				...
				nodeDos.removeClass('elfinderDos-disabled')
			}
		})
		.disable(function() {
			prevEnabled = enabled;
			enabled = false;
			nodeDos.addClass('elfinderDos-disabled');
		})
		.select(function(e) {
		...
		.error(function(e) { 
			var opts  = {
					cssClass  : 'elfinderDos-dialog-error',
			...
			opts.buttons[self.i18n(self.i18n('btnClose'))] = function() { $(this).elfinderDosdialog('close'); };

			self.dialog('<span class="elfinderDos-dialog-icon elfinderDos-dialog-icon-error"/>'+self.i18n(e.data.error), opts);
		})
	...
	// prepare nodeDos
	nodeDos.addClass(this.cssClass)
	...
	this.ui = {
		...
		workzone : $('<div/>').appendTo(nodeDos).elfinderDosworkzone(this),
		// container for folders tree / places
		navbar : $('<div/>').appendTo(nodeDos).elfinderDosnavbar(this),
		// contextmenu
		contextmenu : $('<div/>').appendTo(nodeDos).elfinderDoscontextmenu(this),
		// overlay
		overlay : $('<div/>').appendTo(nodeDos).elfinderDosoverlay({
			show : function() { self.disable(); },
			hide : function() { prevEnabled && self.enable(); }
		}),
		// current folder container
		cwd : $('<div/>').appendTo(nodeDos).elfinderDoscwd(this),
		// notification dialog window
		notify : this.dialog('', {
			cssClass  : 'elfinderDos-dialog-notify',
		...
		statusbar : $('<div class="ui-widget-header ui-helper-clearfix ui-corner-bottom elfinderDos-statusbar"/>').appendTo(nodeDos)
	}
	
	// load required ui
	$.each(this.options.ui || [], function(i, ui) {
		var name = 'elfinderDos'+ui,
			opts = self.options.uiOptions[ui] || {};

		if (!self.ui[ui] && $.fn[name]) {
			self.ui[ui] = $('<'+(opts.tag || 'div')+'/>').appendTo(nodeDos)[name](self, opts);
		}
	});
	
	
	// store instance in nodeDos
	nodeDos[0].elfinderDos = this;
	
	// make nodeDos resizable
	...
	$(document)
		// disable elfinderDos on click outside elfinderDos
		.bind('click.'+this.namespace, function(e) { enabled && !$(e.target).closest(nodeDos).length && self.disable(); })
		// exec shortcuts
...
elfinderDos.prototype = {
		...
		xhr : function(data) { 
			...
			xhr.upload.addEventListener('progress', function(e) {
				...
				if (e.lengthComputable) {
					...
					// reset safari cache (option+command+e), reload elfinderDos page,
	...
	/**
	 * nodeDos for escape html entities in texts
	 * 
	 * @type jQuery
	 */
	_nodeDos : $('<span/>'),
	...
	escape : function(name) {
		return this._nodeDos.text(name).html();
	},
	...
	/**
	 * Compare files based on elfinderDos.sort
	 *
	 * @param  Object  file
	 * @param  Object  file
	 * @return Number
	...
	/**
	 * Sort files based on elfinderDos.sort
	 *
	 * @param  Array  files
	 * @return Array
	 */
	...
	notify : function(opts) {
		var type     = opts.type,
			...
			notify   = ndialog.children('.elfinderDos-notify-'+type),
			ntpl     = '<div class="elfinderDos-notify elfinderDos-notify-{type}"><span class="elfinderDos-dialog-icon elfinderDos-dialog-icon-{type}"/><span class="elfinderDos-notify-msg">{msg}</span> <span class="elfinderDos-notify-cnt"/><div class="elfinderDos-notify-progressbar"><div class="elfinderDos-notify-progress"/></div></div>',
			delta    = opts.cnt,
		...
		if (cnt > 0) {
			!opts.hideCnt && notify.children('.elfinderDos-notify-cnt').text('('+cnt+')');
			ndialog.is(':hidden') && ndialog.elfinderDosdialog('open');
			notify.data('cnt', cnt);
			...
			&& (prc = notify.data('progress')) >= 0) {
				...
				ndialog.find('.elfinderDos-notify-progress')
					.animate({
			...
			notify.remove();
			!ndialog.children().length && ndialog.elfinderDosdialog('close');
		}
	...
	confirm : function(opts) {
		var complete = false,
			options = {
				cssClass  : 'elfinderDos-dialog-confirm',
				modal     : true,
				resizable : false,
				...
				close     : function() { 
					...
					$(this).elfinderDosdialog('destroy');
				}
			},
		...
		if (opts.all) {
			...
			options.open = function() {
				...
				if (width > parseInt(pane.width())) {
					$(this).closest('.elfinderDos-dialog').width(width+30);
				}
			}
		}
		
		return this.dialog('<span class="elfinderDos-dialog-icon elfinderDos-dialog-icon-confirm"/>' + this.i18n(opts.text), options);
	},
	...
	mime2class : function(mime) {
		var prefix = 'elfinderDos-cwd-icon-';
		
		mime = mime.split('/');
		
		return prefix+mime[0]+(mime[0] != 'image' && mime[1] ? ' '+prefix+mime[1].replace(/(\.|\+)/g, '-') : '')
		// return 'elfinderDos-cwd-icon-'+mime.replace('/' , ' elfinderDos-cwd-icon-').replace(/(\.|\+)/g, '-');
	},
	...
	debug : function(type, m) {
		...
		if (d == 'all' || d === true || ($.isArray(d) && $.inArray(type, d) != -1)) {
			window.console && window.console.log && window.console.log('elfinderDos debug: ['+type+'] ['+this.id+']', m);
		} 
		return this;
	},
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/elfinderDos.version.js
 */
 ...
 * @type String
 **/
elfinderDos.prototype.version = '2.0 beta';



/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/jquery.elfinderDos.js
 */

$.fn.elfinderDos = function(o) {
	
	if (o == 'instance') {
		return this.getelfinderDos();
	}
	...
	return this.each(function() {
		...
		if (!this.elfinderDos) {
			new elfinderDos(this, typeof(o) == 'object' ? o : {})
		}
		
		switch(cmd) {
			case 'close':
			case 'hide':
				this.elfinderDos.hide();
				break;
				
			case 'open':
			case 'show':
				this.elfinderDos.show();
				break;
				
			case'destroy':
				this.elfinderDos.destroy();
				break;
		}
...
$.fn.getelfinderDos = function() {
	var instance;
	
	this.each(function() {
		if (this.elfinderDos) {
			instance = this.elfinderDos;
			return false;
		}
	});
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/elfinderDos.options.js
 */

/**
 * Default elfinderDos config
 *
 * @type  Object
 * @autor Dmitry (dio) Levashov
 */
elfinderDos.prototype.options = {
	/**
	 ...
	 * @example
	 *  transport : {
	 *    init : function(elfinderDosInstance) { },
	...
	/**
	 * Event listeners to bind on elfinderDos init
	 *
	 * @type Object
	 * @default  {}
	 */
	...
	/**
	 * Additional css class for filemanager nodeDos.
	 *
	 * @type String
	 */
	cssClass : '',
	 ...
	 * If some required commands will be missed here, elfinderDos will add its
	 *
	 * @type Array
	 */
	commands : [
	 ...
	 * Required to use elfinderDos with WYSIWYG editors etc..
	 *
	 * @type Function
	...
	/**
	 * Make elfinderDos resizable if jquery ui resizable available
	 *
	 * @type Boolean
	 * @default  true
	 */
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/elfinderDos.history.js
 */

/**
 * @class elfinderDos.history
 * Store visited folders
 * and provide "back" and "forward" methods
 *
 * @author Dmitry (dio) Levashov
 */
elfinderDos.prototype.history = function(fm) {
	var self = this,
		/**
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/elfinderDos.command.js
 */

/**
 * elfinderDos command prototype
 *
 * @type  elfinderDos.command
 * @author  Dmitry (dio) Levashov
 */
elfinderDos.prototype.command = function(fm) {
	 ...
	 * this.state = 1;  // command active (for example "fullscreen" command while elfinderDos in fullscreen mode)
	 * @default -1
	 * @type  Number
	 */
	this.state = -1;
	...
	/**
	 * elfinderDos events defaults handlers.
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/elfinderDos.resources.js
 */

/**
 * elfinderDos resources registry.
 * Store shared data
 *
 * @type Object
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.resources = {
	'class' : {
		...
		adroppable  : 'elfinderDos-droppable-active',
		cwdfile     : 'elfinderDos-cwd-file',
		cwd         : 'elfinderDos-cwd',
		tree        : 'elfinderDos-tree',
		treeroot    : 'elfinderDos-navbar-root',
		navdir      : 'elfinderDos-navbar-dir',
		navdirwrap  : 'elfinderDos-navbar-dir-wrapper',
		navarrow    : 'elfinderDos-navbar-arrow',
		navsubtree  : 'elfinderDos-navbar-subtree',
		navcollapse : 'elfinderDos-navbar-collapsed',
		navexpand   : 'elfinderDos-navbar-expanded',
		treedir     : 'elfinderDos-tree-dir',
		placedir    : 'elfinderDos-place-dir',
		searchbtn   : 'elfinderDos-button-search'
	},
	tpl : {
		perms      : '<span class="elfinderDos-perms"/>',
		symlink    : '<span class="elfinderDos-symlink"/>',
		navicon    : '<span class="elfinderDos-nav-icon"/>',
		navspinner : '<span class="elfinderDos-navbar-spinner"/>',
		navdir     : '<div class="elfinderDos-navbar-wrapper"><span id="{id}" class="ui-corner-all elfinderDos-navbar-dir {cssclass}"><span class="elfinderDos-navbar-arrow"/><span class="elfinderDos-navbar-icon"/>{symlink}{permissions}{name}</span><div class="elfinderDos-navbar-subtree"/></div>'
	...
	mixin : {
		make : function() {
			var fm   = this.fm,
				...
				nodeDos = cwd.trigger('create.'+fm.namespace, file).find('#'+id),
			...
			fm.disable();
			nodeDos.find('.elfinderDos-cwd-filename').empty('').append(input.val(file.name));
			input.select().focus();
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/jquery.dialogelfinderDos.js
 */

/**
 * @class dialogelfinderDos - open elfinderDos in dialog window
 *
 * @param  Object  elfinderDos options with dialog options
 * @example
 * $(selector).dialogelfinderDos({
 *     // some elfinderDos options
 ...
 *     destroyOnClose : true        // destroy elfinderDos on close dialog, default = false
 * })
 * @author Dmitry (dio) Levashov
 **/
$.fn.dialogelfinderDos = function(opts) {
	var position = 'elfinderDosPosition',
		destroy  = 'elfinderDosDestroyOnClose';
	
	this.not('.elfinderDos').each(function() {
		
		var doc     = $(document),
			toolbar = $('<div class="ui-widget-header dialogelfinderDos-drag ui-corner-top">'+(opts.title || 'Files')+'</div>'),
			button  = $('<a href="#" class="dialogelfinderDos-drag-close ui-corner-all"><span class="ui-icon ui-icon-closethick"/></a>')
				.appendTo(toolbar)
				.click(function(e) {
					e.preventDefault();
					
					nodeDos.dialogelfinderDos('close');
				}),
			nodeDos    = $(this).addClass('dialogelfinderDos')
				.css('position', 'absolute')
				.hide()
				.appendTo('body')
				.draggable({ handle : '.dialogelfinderDos-drag'})
				.elfinderDos(opts)
				.prepend(toolbar),
			elfinderDos = nodeDos.elfinderDos('instance');
		
		
		nodeDos.width(parseInt(nodeDos.width()) || 840) // fix width if set to "auto"
			.data(destroy, !!opts.destroyOnClose)
			.find('.elfinderDos-toolbar').removeClass('ui-corner-top');
		
		opts.position && nodeDos.data(position, opts.position);
		
		opts.autoOpen !== false && $(this).dialogelfinderDos('open');

	});
	
	if (opts == 'open') {
		var nodeDos = $(this),
			pos  = nodeDos.data(position) || {
				top  : parseInt(($(document).height() - nodeDos.height())/2 - 12),
				left : parseInt(($(document).width() - nodeDos.width())/2)
			},
			zindex = 100;
			
		if (nodeDos.is(':hidden')) {
			
			$('body').find(':visible').each(function() {
				var $this = $(this), z;
				
				if (this !== nodeDos[0] && $this.css('position') == 'absolute' && (z = parseInt($this.zIndex())) > zindex) {
					zindex = z + 1;
				}
			});

			nodeDos.zIndex(zindex).css(pos).show().trigger('resize')

			setTimeout(function() {
				// fix resize icon position and make elfinderDos active
				nodeDos.trigger('resize').mousedown();
			}, 200);
		}
	} else if (opts == 'close') {
		var nodeDos = $(this);
			
		if (nodeDos.is(':visible')) {
			!!nodeDos.data(destroy)
				? nodeDos.elfinderDos('destroy').remove()
				: nodeDos.elfinderDos('close');
		}
	}
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/i18n/elfinderDos.en.js
 */

if (elfinderDos && elfinderDos.prototype && typeof(elfinderDos.prototype.i18) == 'object') {
	elfinderDos.prototype.i18.en = {
		...
		messages   : {
			...
			'errnodeDos'              : 'elfinderDos requires DOM Element to be created.',
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/button.js
 */

/**
 * @class  elfinderDos toolbar button widget.
 * If command has variants - create menu
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDosbutton = function(cmd) {
	return this.each(function() {
		...
		var c        = 'class',
			...
			item     = 'elfinderDos-button-menu-item',
			menu,
			button   = $(this).addClass('ui-state-default elfinderDos-button')
				.attr('title', cmd.title)
				.append('<span class="elfinderDos-button-icon elfinderDos-button-icon-'+cmd.name+'"/>')
		...
		if ($.isArray(cmd.variants)) {
			button.addClass('elfinderDos-menubutton');
			
			menu = $('<div class="ui-widget ui-widget-content elfinderDos-button-menu ui-corner-all"/>')
				.hide()
				.appendTo(button)
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/contexmenu.js
 */

/**
 * @class  elfinderDos contextmenu
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDoscontextmenu = function(fm) {
	
	return this.each(function() {
		var event        = $.browser.opera ? 'click' : 'contextmenu',
			c            = 'class',
			clItem       = 'elfinderDos-contextmenu-item',
			clGroup      = 'elfinderDos-contextmenu-group',
			clSub        = 'elfinderDos-contextmenu-sub',
			...
			menu = $(this).addClass('ui-helper-reset ui-widget ui-state-default ui-corner-all elfinderDos-contextmenu elfinderDos-contextmenu-'+fm.direction)
				.hide()
				.appendTo('body')
			...
			append = function(type, targets) {
				...
				$.each(commands, function(i, name) {
					...
					if (!(cmd && cmd.name)) {
						if (name == '|' && sep) {
							menu.append('<div class="elfinderDos-contextmenu-separator"/>');
							sep = false;
						}
						return;
					}
					...
					item = $('<div class="'+clItem+'"><span class="elfinderDos-button-icon elfinderDos-button-icon-'+cmd.name+' elfinderDos-contextmenu-icon"/><span>'+cmd.title+'</span></div>')
						.data({cmd : cmd.name});
					...
					if (cmd.variants) {
						...
						sub = $('<div class="ui-corner-all '+clSub+'"/>')
							.appendTo(item.addClass(clGroup).append('<span class="elfinderDos-contextmenu-arrow"/>'));
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/cwd.js
 */

/**
 * elfinderDos current working directory ui.
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDoscwd = function(fm) {
	
	this.not('.elfinderDos-cwd').each(function() {
		// fm.time('cwdLoad');
		var 
			...
			/**
			 * Css class for temporary nodeDoss (for mkdir/mkfile) commands
			 *
			 * @type String
			 **/
			clTmp = clFile+'-tmp',
			 ...
			 **/
			templates = {
				icon : '<div id="{hash}" class="'+clFile+' {permsclass} {dirclass} ui-corner-all"><div class="elfinderDos-cwd-file-wrapper ui-corner-all"><div class="elfinderDos-cwd-icon {mime} ui-corner-all" unselectable="on" {style}/>{marker}</div><div class="elfinderDos-cwd-filename" title="{name}">{name}</div></div>',
				row  : '<tr id="{hash}" class="'+clFile+' {permsclass} {dirclass}"><td><div class="elfinderDos-cwd-file-wrapper"><span class="elfinderDos-cwd-icon {mime}"/>{marker}<span class="elfinderDos-cwd-filename">{name}</span></div></td><td>{perms}</td><td>{date}</td><td>{size}</td><td>{kind}</td></tr>'
			},
			...
			/**
			 * Fire elfinderDos "select" event and pass selected files to it
			 *
			 * @return void
			 */
			trigger = function() {
			 ...
			 *
			 * @param DOMElement  file/dir nodeDos
			 * @return void
			 */
			...
			makeDroppable = function() {
				setTimeout(function() {
					cwd.find('.directory:not(.'+clDroppable+',.elfinderDos-na,.elfinderDos-ro)').droppable(fm.droppable);
				}, 20);
			},
			...
			attachThumbnails = function(images) {
				...
				$.each(images, function(hash, tmb) {
					var nodeDos = cwd.find('#'+hash);

					if (nodeDos.length) {

						(function(nodeDos, tmb) {
							$('<img/>')
								.load(function() { nodeDos.find('.elfinderDos-cwd-icon').css('background', "url('"+tmb+"') center center no-repeat"); })
								.attr('src', tmb);
						})(nodeDos, url+tmb);
					} else {
						ret = false;
			...
			add = function(files) {
				...
				while (l--) {
					...
					if ((nodeDos = findnodeDos(file)) && nodeDos.length) {
						nodeDos.before(itemhtml(file));
			...
			content = function(files, any) {
				...
				cwd.removeClass('elfinderDos-cwd-view-icons elfinderDos-cwd-view-list')
					.addClass('elfinderDos-cwd-view-'+(list ? 'list' :'icons'));
			...
			cwd = $(this)
				...
				parent = $(this).parent().resize(function() {
					...
					cwd.siblings('.elfinderDos-panel:visible').each(function() {
				...
				// workzone nodeDos
				wz = parent.children('.elfinderDos-workzone').append(this),
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/dialog.js
 */

/**
 * @class  elfinderDos dialog
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDosdialog = function(opts) {
	var dialog;
	...
	opts = $.extend({}, $.fn.elfinderDosdialog.defaults, opts);

	this.filter(':not(.ui-dialog-content)').each(function() {
		var self       = $(this).addClass('ui-dialog-content ui-widget-content'),
			parent     = self.parent(),
			clactive   = 'elfinderDos-dialog-active',
			cldialog   = 'elfinderDos-dialog',
			clnotify   = 'elfinderDos-dialog-notify',
			clhover    = 'ui-state-hover',
			id         = parseInt(Math.random()*1000000),
			overlay    = parent.children('.elfinderDos-overlay'),
			...
			dialog = $('<div class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable std42-dialog  '+cldialog+' '+opts.cssClass+'"/>')
				...
				.bind('open', function() {
					opts.modal && overlay.elfinderDosoverlay('show');
				...
				.bind('close', function() {
					var dialogs = parent.find('.elfinderDos-dialog:visible'),
						z = maxZIndex();
					
					opts.modal && overlay.elfinderDosoverlay('hide');
		...
		opts.autoOpen && self.elfinderDosdialog('open');
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/navbar.js
 */

/**
 * @class elfinderDosnav - elfinderDos container for diretories tree and places
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDosnavbar = function(fm) {
	
	this.not('.elfinderDos-navbar').each(function() {
		var nav    = $(this).addClass('ui-state-default elfinderDos-navbar'),
			parent = nav.parent()
				.resize(function() {
					nav.height(wz.height() - delta);
				}),
			wz     = parent.children('.elfinderDos-workzone').append(nav),
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/overlay.js
 */


$.fn.elfinderDosoverlay = function(opts) {
	
	this.filter(':not(.elfinderDos-overlay)').each(function() {
		opts = $.extend({}, opts);
		$(this).addClass('ui-widget-overlay elfinderDos-overlay')
			.hide()
			.mousedown(function(e) {
				e.preventDefault();
				e.stopPropagation();
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/panel.js
 */

$.fn.elfinderDospanel = function(fm) {
	
	return this.each(function() {
		var panel = $(this).addClass('elfinderDos-panel ui-state-default ui-corner-all'),
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/path.js
 */

/**
 * @class elfinderDos ui
 * Display current folder path in statusbar.
 * Click on folder name in path - open folder
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDospath = function(fm) {
	return this.each(function() {
		var path = $(this).addClass('elfinderDos-path').html('&nbsp;')
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/places.js
 */

/**
 * @class elfinderDos places/favorites ui
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDosplaces = function(fm, opts) {
	return this.each(function() {
		var dirs      = [],
			c         = 'class',
			...
			/**
			 * Convert places dir nodeDos into dir hash
			 *
			...
			/**
			 * Return nodeDos for given dir object
			 *
			...
			/**
			 * Add new nodeDos into places
			 *
			 * @param  Object  directory object
			 * @return void
			 **/
			add = function(dir) {
				var nodeDos = create(dir);
				...
				if (subtree.children().length) {
					$.each(subtree.children(), function() {
						...
						if (dir.name.localeCompare(current.children('.'+navdir).text()) < 0) {
							return !nodeDos.insertBefore(current);
						}
					});
				} 
				
				dirs.push(dir.hash);
				!nodeDos.parent().length && subtree.append(nodeDos);
				...
				nodeDos.draggable({
					...
					helper   : function() {
						...
						return $('<div class="elfinderDos-place-drag elfinderDos-'+fm.direction+'"/>')
								.append(dir.clone())
			...
			/**
			 * nodeDos - wrapper for places root
			 *
			 * @type jQuery
			 **/
			wrapper = create({
			 ...
			 * @type jQuery
			 **/
			places = $(this).addClass(fm.res(c, 'tree')+' elfinderDos-places ui-corner-all')
				.hide()
				.append(wrapper)
				...
				.droppable({
					tolerance  : 'pointer',
					accept     : '.elfinderDos-cwd-file-wrapper,.elfinderDos-tree-dir,.elfinderDos-cwd-file',
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/searchbutton.js
 */

/**
 * @class  elfinderDos toolbar search button widget.
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDossearchbutton = function(cmd) {
	return this.each(function() {
		var result = false,
			button = $(this).hide().addClass('ui-widget-content elfinderDos-button '+cmd.fm.res('class', 'searchbtn')+''),
			search = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/stat.js
 */
 ...
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDosstat = function(fm) {
	return this.each(function() {
		var size       = $(this).addClass('elfinderDos-stat-size'),
			sel        = $('<div class="elfinderDos-stat-selected"/>'),
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/toolbar.js
 */

/**
 * @class  elfinderDos toolbar
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDostoolbar = function(fm, opts) {
	this.not('.elfinderDos-toolbar').each(function() {
		var commands = fm._commands,
			self     = $(this).addClass('ui-helper-clearfix ui-widget-header ui-corner-top elfinderDos-toolbar'),
			panels   = opts || [],
			l        = panels.length,
			i, cmd, panel, button;
		...
		while (l--) {
			panel = $('<div class="ui-widget-content ui-corner-all elfinderDos-buttonset"/>');
			i = panels[l].length;
			while (i--) {
				if ((cmd = commands[panels[l][i]])) {
					button = 'elfinderDos'+cmd.options.ui;
					$.fn[button] && panel.prepend($('<div/>')[button](cmd));
				}
			}
			
			panel.children().length && self.prepend(panel);
			panel.children(':not(:last),:not(:first):not(:last)').after('<span class="ui-widget-content elfinderDos-toolbar-button-separator"/>');
		}
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/tree.js
 */

/**
 * @class  elfinderDos folders tree
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDostree = function(fm, opts) {
	...
	this.not('.'+treeclass).each(function() {
		...
		var c = 'class',
			 ...
			 * @type String
			 */
			loaded    = 'elfinderDos-subtree-loaded',
			...
			/**
			 * Find directory (wrapper) in required nodeDos
			...
			findSibling = function(subtree, dir) {
				var nodeDos = subtree.children(':first'),
					info;

				while (nodeDos.length) {
					if ((info = fm.file(fm.navId2Hash(nodeDos.children('[id]').attr('id')))) 
					&& dir.name.localeCompare(info.name) < 0) {
						return nodeDos;
					}
					nodeDos = nodeDos.next();
				}
				return $('');
			},
			...
			sync = function() {
				...
				if (openRoot) {
					rootnodeDos = tree.find('#'+fm.navHash2Id(fm.root()));
					rootnodeDos.is('.'+loaded) && rootnodeDos.addClass(expanded).next('.'+subtree).show();
					openRoot = false;
				}
			...
			updateDroppable = function() {
				tree.find('.'+navdir+':not(.'+droppable+',.elfinderDos-ro,.elfinderDos-na)').droppable(droppableopts);
			},
			...
			tree = $(this).addClass(treeclass)
				...
				.delegate('.'+navdir, 'hover', function(e) {
					...
					if (!link.is('.'+dropover+' ,.'+disabled)) {
						enter && !link.is('.'+root+',.'+draggable+',.elfinderDos-na,.elfinderDos-wo') && link.draggable(fm.draggable);
		...
		tree.parent().find('.elfinderDos-navbar').append(tree).show();
		...
		.change(function(e) {
			var dirs = filter(e.data.changed),
				l    = dirs.length,
				dir, nodeDos, tmp, realParent, reqParent, realSibling, reqSibling, isExpanded, isLoaded;
			
			while (l--) {
				dir = dirs[l];
				if ((nodeDos = tree.find('#'+fm.navHash2Id(dir.hash))).length) {
					if (dir.phash) {
						realParent  = nodeDos.closest('.'+subtree);
						reqParent   = findSubtree(dir.phash);
						realSibling = nodeDos.parent().next();
						...
						if (reqParent[0] !== realParent[0] || realSibling.get(0) !== reqSibling.get(0)) {
							reqSibling.length ? reqSibling.before(nodeDos) : reqParent.append(nodeDos);
						}
					}
					isExpanded = nodeDos.is('.'+expanded);
					isLoaded   = nodeDos.is('.'+loaded);
					tmp        = $(itemhtml(dir));
					nodeDos.replaceWith(tmp.children('.'+navdir));
					
					if (dir.dirs 
					&& (isExpanded || isLoaded) 
					&& (nodeDos = tree.find('#'+fm.navHash2Id(dir.hash))) 
					&& nodeDos.next('.'+subtree).children().length) {
						isExpanded && nodeDos.addClass(expanded);
						isLoaded && nodeDos.addClass(loaded);
					}
				}
			}
		...
		.remove(function(e) {
			...
			while (l--) {
				if ((nodeDos = tree.find('#'+fm.navHash2Id(dirs[l]))).length) {
					stree = nodeDos.closest('.'+subtree);
					nodeDos.parent().detach();
					if (!stree.children().length) {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/uploadButton.js
 */

/**
 * @class  elfinderDos toolbar's button tor upload file
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDosuploadbutton = function(cmd) {
	return this.each(function() {
		var button = $(this).elfinderDosbutton(cmd)
				.unbind('click'), 
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/viewbutton.js
 */

/**
 * @class  elfinderDos toolbar button to switch current directory view.
 *
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDosviewbutton = function(cmd) {
	return this.each(function() {
		var button = $(this).elfinderDosbutton(cmd),
			icon   = button.children('.elfinderDos-button-icon');

		cmd.change(function() {
			var icons = cmd.value == 'icons';

			icon.toggleClass('elfinderDos-button-icon-view-list', icons);
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/ui/workzone.js
 */

/**
 * @class elfinderDosworkzone - elfinderDos container for nav and current directory
 * @author Dmitry (dio) Levashov
 **/
$.fn.elfinderDosworkzone = function(fm) {
	var cl = 'elfinderDos-workzone';
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/archive.js
 */

/**
 * @class  elfinderDos command "archive"
 * Archive selected files
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.archive = function() {
	var self  = this,
		fm    = self.fm,
		mimes = [];
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/back.js
 */

/**
 * @class  elfinderDos command "back"
 * Open last visited folder
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.back = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/copy.js
 */

/**
 * @class elfinderDos command "copy".
 * Put files in filemanager clipboard.
 *
 * @type  elfinderDos.command
 * @author  Dmitry (dio) Levashov
 */
elfinderDos.prototype.commands.copy = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/cut.js
 */
 ...
 * @author  Dmitry (dio) Levashov
 */
elfinderDos.prototype.commands.cut = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/download.js
 */

/**
 * @class elfinderDos command "download". 
 * Download selected files.
 * Only for new api
 *
 * @author Dmitry (dio) Levashov, dio@std42.ru
 **/
elfinderDos.prototype.commands.download = function() {
	var self   = this,
		fm     = this.fm,
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/duplicate.js
 */

/**
 * @class elfinderDos command "duplicate"
 * Create file/folder copy with suffix "copy Number"
 *
 * @type  elfinderDos.command
 * @author  Dmitry (dio) Levashov
 */
elfinderDos.prototype.commands.duplicate = function() {
	var fm = this.fm;
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/edit.js
 */

/**
 * @class elfinderDos command "edit". 
 * Edit text file in dialog window
 *
 * @author Dmitry (dio) Levashov, dio@std42.ru
 **/
elfinderDos.prototype.commands.edit = function() {
	var self  = this,
		fm    = this.fm,
		...
		edit = function(file) {
			var hash   = file.hash,
				...
				dialog = function(text) {
					var editor = $('<textarea class="elfinderDos-file-edit" rows="20">'+text+'</textarea>')
							.keydown(function(e) {
								var code = e.keyCode,
									value, start;
					...
					cancel = function() {
						dfrd.resolve();
						editor.elfinderDosdialog('close');
					},
					save = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/extract.js
 */

/**
 * @class  elfinderDos command "extract"
 * Extract files from archive
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.extract = function() {
	var self    = this,
		fm      = self.fm,
		mimes   = [],
		filter  = function(files) {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/forward.js
 */

/**
 * @class  elfinderDos command "forward"
 * Open next visited folder
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.forward = function() {
	this.alwaysEnabled = true;
	this.updateOnSelect = true;
	this.shortcuts = [{
		pattern     : 'ctrl+right'
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/getfile.js
 */

/**
 * @class elfinderDos command "getfile". 
 * Return selected files info into outer callback.
 * For use elfinderDos with wysiwyg editors etc.
 *
 * @author Dmitry (dio) Levashov, dio@std42.ru
 **/
elfinderDos.prototype.commands.getfile = function() {
	var self   = this,
		fm     = this.fm,
		filter = function(files) {
			var o = self.options;
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/help.js
 */

/**
 * @class  elfinderDos command "help"
 * "About" dialog
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.help = function() {
	var fm = this.fm,
		content;
	...
	this.fm.one('open', function() {
		setTimeout(function() {
			var linktpl = '<div class="elfinderDos-help-link"> <a href="{url}">{link}</a></div>',
				atpl    = '<div class="elfinderDos-help-team"><div>{author}</div>{work}</div>',
				url     = /\{url\}/,
				link    = /\{link\}/,
				author  = /\{author\}/,
				work    = /\{work\}/,
				...
				lic     = 'elfinderDos-help-license',
				tab     = '<li class="ui-state-default ui-corner-top"><a href="#{id}">{title}</a></li>',
				html    = ['<div class="ui-tabs ui-widget ui-widget-content ui-corner-all elfinderDos-help">', 
						'<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">'],
				stpl    = '<div class="elfinderDos-help-shortcut"><div class="elfinderDos-help-shortcut-pattern">{pattern}</div> {descrip}</div>',
				sep     = '<div class="elfinderDos-help-separator"/>',
			...
			// main tab
			html.push('<div id="main" class="ui-tabs-panel ui-widget-content ui-corner-bottom"><div class="elfinderDos-help-logo"/>')
			html.push('<h3>elfinderDos</h3>');
			...
			html.push(linktpl[r](url, 'http://elfinderDos.org/')[r](link, fm.i18n('homepage')));
			html.push(linktpl[r](url, 'https://github.com/Studio-42/elfinderDos/wiki')[r](link, fm.i18n('docs')));
			html.push(linktpl[r](url, 'https://github.com/Studio-42/elfinderDos')[r](link, fm.i18n('github')));
			html.push(linktpl[r](url, 'http://twitter.com/elrte_elfinderDos')[r](link, fm.i18n('twitter')));
			...
			if (shortcuts.length) {
				html.push('<div class="ui-widget-content elfinderDos-help-shortcuts">');
				$.each(shortcuts, function(i, s) {
				...
				html.push('<div class="elfinderDos-help-disabled">'+fm.i18n('shortcutsof')+'</div>')
			}
			...
			html.push('<a href="http://elrte.org/redmine/projects/elfinderDos/boards" target="_blank" class="elfinderDos-dont-panic"><span>DON\'T PANIC</span></a>');
			html.push('</div>')
			// end help
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/home.js
 */


elfinderDos.prototype.commands.home = function() {
	this.title = 'Home';
	this.alwaysEnabled  = true;
	this.updateOnSelect = false;
	this.shortcuts = [{
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/info.js
 */

/**
 * @class elfinderDos command "info". 
 * Display dialog with file properties.
 *
 * @author Dmitry (dio) Levashov, dio@std42.ru
 **/
elfinderDos.prototype.commands.info = function() {
	var m   = 'msg',
		fm  = this.fm,
		spclass = 'elfinderDos-info-spinner',
		msg = {
			calc     : fm.i18n('calc'),
			size     : fm.i18n('size'),
	...
	this.tpl = {
		main       : '<div class="ui-helper-clearfix elfinderDos-info-title"><span class="elfinderDos-cwd-icon {class} ui-corner-all"/>{title}</div><table class="elfinderDos-info-tb">{content}</table>',
		itemTitle  : '<strong>{name}</strong><span class="elfinderDos-info-kind">{kind}</span>',
	...
	this.exec = function(hashes) {
		var self    = this,
			...
			opts    = {
				...
				// test in ie >_<
				width : 'auto',
				close : function() { $(this).elfinderDosdialog('destroy'); }
			},
			count = [],
		...
		if (cnt == 1) {
			...
			view  = view.replace('{class}', 'elfinderDos-cwd-icon-group');
		...
		if (tmb) {
			$('<img/>')
				.load(function() { dialog.find('.elfinderDos-cwd-icon').css('background', 'url("'+tmb+'") center center no-repeat'); })
				.attr('src', tmb);
		}
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/mkdir.js
 */

/**
 * @class  elfinderDos command "mkdir"
 * Create new folder
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.mkdir = function() {
	this.disableOnSearch = true;
	this.updateOnSelect  = false;
	this.mime            = 'directory';
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/mkfile.js
 */

/**
 * @class  elfinderDos command "mkfile"
 * Create new empty file
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.mkfile = function() {
	this.disableOnSearch = true;
	this.updateOnSelect  = false;
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/open.js
 */

/**
 * @class  elfinderDos command "open"
 * Enter folder or open files in new windows
 *
 * @author Dmitry (dio) Levashov
 **/  
elfinderDos.prototype.commands.open = function() {
	this.alwaysEnabled = true;
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/paste.js
 */

/**
 * @class  elfinderDos command "paste"
 * Paste filesfrom clipboard into directory.
 * If files pasted in its parent directory - files duplicates will created
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.paste = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/quicklook.js
 */

/**
 * @class  elfinderDos command "quicklook"
 * Fast preview for some files types
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.quicklook = function() {
	var self       = this,
		fm         = self.fm,
		/**
		 * window closed state
		 ...
		 * @type Number
		 **/
		navicon    = 'elfinderDos-quicklook-navbar-icon',
		/**
		 * navbar "fullscreen" icon class
		 *
		 * @type Number
		 **/
		fullscreen  = 'elfinderDos-quicklook-fullscreen',
		/**
		 ...
		 *
		 * @param  jQuery  file nodeDos in cwd
		 * @return void
		 **/
		closedCss = function(nodeDos) {
			return {
				opacity : 0,
				width   : 20,//nodeDos.width(),
				height  : fm.view == 'list' ? 1 : 20,
				top     : nodeDos.offset().top+'px', 
				left    : nodeDos.offset().left+'px' 
			}
		},
		/**
		...
		parent, 
		/**
		 * elfinderDos current directory nodeDos
		 *
		 * @type jQuery
		 **/
		cwd, 
		title   = $('<div class="elfinderDos-quicklook-title"/>'),
		icon    = $('<div/>'),
		info    = $('<div class="elfinderDos-quicklook-info"/>'),//.hide(),
		...
		navbar  = $('<div class="elfinderDos-quicklook-navbar"/>')
			...
			.append('<div class="elfinderDos-quicklook-navbar-separator"/>')
			.append($('<div class="'+navicon+' '+navicon+'-close"/>').mousedown(function() { self.window.trigger('close'); }))
		;

	this.resize = 'resize.'+fm.namespace;
	this.info = $('<div class="elfinderDos-quicklook-info-wrapper"/>')
		.append(icon)
		.append(info);
		
	this.preview = $('<div class="elfinderDos-quicklook-preview ui-helper-clearfix"/>')
		// clean info/icon
		...
		.bind('update', function(e) {
			var fm      = self.fm,
				...
				file    = e.file,
				tpl     = '<div class="elfinderDos-quicklook-info-data">{value}</div>',
				tmb;
			...
			if (file) {
				...
				icon.addClass('elfinderDos-cwd-icon ui-corner-all '+fm.mime2class(file.mime));
	...
	this.window = $('<div class="ui-helper-reset ui-widget elfinderDos-quicklook" style="position:absolute"/>')
		.click(function(e) { e.stopPropagation(); })
		.append(
			$('<div class="elfinderDos-quicklook-titlebar"/>')
				.append(title)
		...
		.draggable({handle : 'div.elfinderDos-quicklook-titlebar'})
		.bind('open', function(e) {
			var win  = self.window, 
				file = self.value,
				nodeDos;

			if (self.closed() && file && (nodeDos = cwd.find('#'+file.hash)).length) {
				
				state = animated;
				nodeDos.trigger('scrolltoview');
				win.css(closedCss(nodeDos))
					.show()
		...
		.bind('close', function(e) {
			var win     = self.window,
				...
				file    = self.value,
				nodeDos    = cwd.find('#'+win.data('hash')),
				close   = function() {
					state = closed;
					win.hide();
			...
			if (self.opened()) {
				...
				nodeDos.length
					? win.animate(closedCss(nodeDos), 500, close)
					: close();
			}
		});
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/quicklook.plugins.js
 */


elfinderDos.prototype.commands.quicklook.plugins = [
	
	/**
	 * Images preview plugin
	 *
	 * @param elfinderDos.commands.quicklook
	 **/
	function(ql) {
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if ($.inArray(file.mime, mimes) !== -1) {
				...
				.done(function(data) {
					ql.hideinfo();
					doc = $('<iframe class="elfinderDos-quicklook-preview-html"/>').appendTo(preview)[0].contentWindow.document;
					doc.open();
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if (mime.indexOf('text/') === 0 || $.inArray(mime, mimes) !== -1) {
				...
				.done(function(data) {
					ql.hideinfo();
					$('<div class="elfinderDos-quicklook-preview-text-wrapper"><pre class="elfinderDos-quicklook-preview-text">'+fm.escape(data.content)+'</pre></div>').appendTo(preview);
				});
			}
		});
	},
	...
	function(ql) {
		...
		active && preview.bind('update', function(e) {
			var file = e.file, nodeDos;
			
			if (file.mime == mime) {
				e.stopImmediatePropagation();
				preview.one('change', function() {
					nodeDos.unbind('load').remove();
				});
				
				nodeDos = $('<iframe class="elfinderDos-quicklook-preview-pdf"/>')
					.hide()
					.appendTo(preview)
					.load(function() { 
						ql.hideinfo();
	...
	function(ql) {
		...
		active && preview.bind('update', function(e) {
			...
			if (file.mime == mime) {
				e.stopImmediatePropagation();
				ql.hideinfo();
				preview.append((nodeDos = $('<embed class="elfinderDos-quicklook-preview-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" src="'+fm.url(file.hash)+'" quality="high" type="application/x-shockwave-flash" />')));
			}
		});
	},
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if (ql.support.audio[type]) {
				...
				nodeDos = $('<audio class="elfinderDos-quicklook-preview-audio" controls preload="auto" autobuffer><source src="'+ql.fm.url(file.hash)+'" /></audio>')
					.appendTo(preview);
				autoplay && nodeDos[0].play();
			}
		}).bind('change', function() {
			if (nodeDos && nodeDos.parent().length) {
				nodeDos[0].pause();
				nodeDos.remove();
				nodeDos= null;
			}
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if (ql.support.video[type]) {
				...
				ql.hideinfo();
				nodeDos = $('<video class="elfinderDos-quicklook-preview-video" controls preload="auto" autobuffer><source src="'+ql.fm.url(file.hash)+'" /></video>').appendTo(preview);
				autoplay && nodeDos[0].play();
	...
	function(ql) {
		...
		preview.bind('update', function(e) {
			...
			if ($.inArray(file.mime, mimes) !== -1) {
				...
				nodeDos = $('<embed src="'+ql.fm.url(file.hash)+'" type="'+mime+'" class="elfinderDos-quicklook-preview-'+(video ? 'video' : 'audio')+'"/>')
					.appendTo(preview);
			}
		}).bind('change', function() {
			if (nodeDos && nodeDos.parent().length) {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/reload.js
 */

/**
 * @class  elfinderDos command "reload"
 * Sync files and folders
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.reload = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/rename.js
 */

/**
 * @class elfinderDos command "rename". 
 * Rename selected file.
 *
 * @author Dmitry (dio) Levashov, dio@std42.ru
 **/
elfinderDos.prototype.commands.rename = function() {
	this.disableOnSearch = true;
	...
	this.exec = function() {
		var fm       = this.fm,
			...
			filename = '.elfinderDos-cwd-filename',
			dfrd     = $.Deferred()
				.fail(function(error) {
			...
			nodeDos = cwd.find('#'+file.hash).find(filename).empty().append(input.val(file.name));
		
		if (!file || cnt > 1 || !nodeDos.length) {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/rm.js
 */

/**
 * @class  elfinderDos command "rm"
 * Delete files
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.rm = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/search.js
 */

/**
 * @class  elfinderDos command "search"
 * Find files
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.search = function() {
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/up.js
 */

/**
 * @class  elfinderDos command "up"
 * Go into parent directory
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.up = function() {
	this.alwaysEnabled = true;
	this.updateOnSelect = false;
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/upload.js
 */

/**
 * @class elfinderDos command "upload"
 ...
 * @author  Dmitry (dio) Levashov
 */
elfinderDos.prototype.commands.upload = function() {
	...
	this.exec = function(data) {
		var fm = this.fm,
			upload = function(data) {
				dialog.elfinderDosdialog('close');
				fm.upload(data)
					.fail(function(error) {
						dfrd.reject(error);
					})
		...
		dialog = $('<div class="elfinderDos-upload-dialog-wrapper"/>')
			.append(button);
		
		if (fm.dragUpload) {
			dropbox = $('<div class="ui-corner-all elfinderDos-upload-dropbox">'+fm.i18n('dropFiles')+'</div>')
				.prependTo(dialog)
				.after('<div class="elfinderDos-upload-dialog-or">'+fm.i18n('or')+'</div>')[0];
...
/*
 * File: /home/troex/Sites/git/elfinderDos-2.x/js/commands/view.js
 */

/**
 * @class  elfinderDos command "view"
 * Change current directory view (icons/list)
 *
 * @author Dmitry (dio) Levashov
 **/
elfinderDos.prototype.commands.view = function() {
