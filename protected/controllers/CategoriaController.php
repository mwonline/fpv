...
class CategoriaController extends Controller {
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Categoria'])) {
            $model->attributes = $_POST['Categoria'];
            $modelAnterior = Categoria::model()->findByPk($model->idcategoria);
            if ($modelAnterior->alias != $model->alias) {
                $modelAlias = new AliasCategoria;
                $datos=  array(
                    'alias'=>$modelAnterior->alias,
                    'idCategoria'=>$modelAnterior->idcategoria,
                );
                $modelAlias->attributes=$datos;
                $modelAlias->save();
            }
            if ($model->save())
                $this->redirect(array('view', 'id' => $model->idcategoria));
